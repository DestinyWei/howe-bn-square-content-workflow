...
`v1.1.3` 修复占位符下方紧跟 H2 标题时，图片粘贴插入点落在块级边界导致批量插图全部失败的问题。助手现在会先在占位符上方创建普通段落锚点，把光标放入锚点内部再粘贴图片，避免继承标题格式。

### v1.1.2

`v1.1.2` 修复 `v1.1.1` 中过早移动币安临时上传图片块导致上传被打断的问题。批量插图助手现在会等待图片真实加载完成后再移动到占位符上方，并会清理失败重试时遗留的空“标题”块。

### v1.1.1

`v1.1.1` 修复币安编辑器把新上传图片先插到正文末尾时，批量插图助手未能及时纠偏的问题。助手现在会在检测到新增图片块后先移动到目标占位符前，再等待图片上传完成。
...
- 自动移除或合并 X/Twitter 链接和 `@handle`，避免搬运到币安广场后格式异常。
...
首个正式版本，提供 X Article 采集、排版器转换、币安广场文章草稿填入和基础图片占位流程。
...
如果币安编辑器在图片和占位符之间生成了空白编辑块，助手会在验证时只清理这段图片区间内的空白块，避免图片下方出现异常大段留白。
