...
把 X Article 迁移到币安广场文章编辑器的本地工作流工具：

`X Article 采集 -> 币安格式转换 -> 兼容性检查 -> 填入草稿 -> 批量插图 -> 人工审核 -> 手动发布`
...
- `content-x.js`：X Article 页面采集脚本。
- `content-binance.js`：币安广场草稿填充与插图助手。
...
2. 尝试把对应图片插入到占位符前方。
3. 等待图片真实加载完成，失败时自动清理损坏图片框并重试一次。
4. 保留所有占位符，用于后续人工审核。
5. 自动进入审核模式。

每张图片插入后都会重新验证实际位置。如果币安编辑器沿用了旧光标、导致图片落到前一个占位符，助手会自动把新增图片纠偏到对应占位符前。

重复点击“一键自动插入”时，本轮已经成功的图片会被跳过，只重试失败项；意外产生的同编号重复占位符也会自动合并。
